...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where('actor_id = 6 AND ord != 1')
...
def biggest_cast
  ...
  # Movie.select('movie.id', title:, 'castings.*').joins(:castings).group('movie_id').count
...
def directed_by_one_of(them)
  ...
  # Find the id and title of all the movies directed by one of 'the'.
